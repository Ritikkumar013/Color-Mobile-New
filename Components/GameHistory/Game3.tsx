...
const HistoryTable: React.FC = () => {
  ...
  const fetchHistory = async (isRefresh = false) => {
    try {
      ...
      // 4. IMPORTANT: Filter to only include the specific game you requested (e.g., ending with '-3')
      const requestedPeriodGames = filteredGames.filter((item) =>
        item.period.endsWith("-3")
      );
      ...
      if (sortedRequestedGames.length > 0) {
        ...
        // The start time of the *next* game for this period (-3)
        const nextStartTime = latestStartTime + durationMs;
        
        // The end time of the *next* game for this period (-3)
      ...
      {data.length === 0 ? (
        <View className="bg-gray-100 rounded-lg p-8 items-center">
          <Text className="text-gray-600 text-center text-base">
            No game history available yet for period ending in -.
          </Text>
