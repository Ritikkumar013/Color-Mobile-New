...
//         "https://ctbackend.crobstacle.com/api/users/profile", // Update with your actual profile endpoint
//         {
//           method: "GET",
//           headers: {
...
//               if (token) {
//                 try {
//                   await fetch("https://ctbackend.crobstacle.com/api/auth/logout", {
//                     method: "POST",
//                     headers: {
//                       Authorization: `Bearer ${token}`,
//                       "Content-Type": "application/json",
...
import AsyncStorage from "@react-native-async-storage/async-storage";
import { useAuth } from "@/Components/Auth";
import { useSocket } from "../../Components/context/SocketContext"; // Import from context
...
const Account = () => {
    // 1. USE SOCKET CONTEXT BALANCE AND REFRESH FUNCTION
    // This balance is automatically updated via the socket
    const { 
        balance, // Real-time balance from context
        // isConnected, // (Optional) can be used for status
        refreshBalance // Unified function to manually fetch balance (API/socket)
    } = useSocket();

    // 2. Remove the local balance state: 
    // const [balance, setBalance] = useState<number | null>(null); 
    
    // Keep a local loading state for the manual refresh indicator
    const [loading, setLoading] = useState(false); 
    const [userProfile, setUserProfile] = useState<UserProfile | null>(null);
    const [profileLoading, setProfileLoading] = useState(false);
    const [logoutLoading, setLogoutLoading] = useState(false);
    ...
    // 3. REMOVE THE REDUNDANT fetchBalance API function
    /* const fetchBalance = async () => { ... }; 
    */ 
    
    // 4. Create a unified manual refresh handler (renamed for clarity)
    const handleManualBalanceRefresh = async () => {
        if (loading) return; // Prevent double click

        setLoading(true);
        try {
            // Use the context function to trigger the balance fetch (via API or socket)
            ...
            // Alert.alert("Error", "Failed to refresh balance.");
             ...
             // Stop local loading indicator after a short delay for UX
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            // Reset local component state
                            // setBalance(null); // No longer needed as context will reset
                            setUserProfile(null);
    ...
    useEffect(() => {
        // 5. Initial fetch is now just for profile, as balance is handled by SocketContext on mount/connection
        // You can keep refreshBalance() here if you want an *immediate* API fetch on mount for robustness
        // fetchBalance(); // <-- REMOVED
        refreshBalance(); // Use the context's refresh to ensure up-to-date balance on initial load
        fetchUserProfile();
    }, []);
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                ...
                <View className="bg-white p-4 rounded-xl flex flex-col shadow-md mx-4 -mt-12">
                    <View className="flex flex-row justify-between items-center shadow-md">
                        ...
                        <TouchableOpacity
                            className="flex flex-row items-center"
                            // 6. Call the new manual refresh handler
                            onPress={handleManualBalanceRefresh} 
                        >
                            {loading ? (
                                <ActivityIndicator size="small" color="#16a34a" />
                            ) : (
                                <>
                                    {/* 7. Display the balance from the context */}
                                    <Text className="text-green-600 font-bold text-xl">
