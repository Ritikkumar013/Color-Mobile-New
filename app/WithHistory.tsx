...
const TransactionItem = ({ item }: { item: TransactionDetail }) => {
  ...
  return (
    <View className="flex-row justify-between items-center py-3 border-b border-gray-100 last:border-b-0">
      ...
      {/* Amount Text - Always red for withdrawals */}
...
const WithHistory = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        // Extract the nested data array and filter only debit transactions with money category
        if (data.status === "success" && data.data && Array.isArray(data.data.data)) {
          // Filter only debit transactions (withdrawals) from money category (no game bets)
          const debitTransactions = data.data.data.filter(
            (transaction) => transaction.type === 'debit' && transaction.category === 'money'
          );
          setAllTransactions(debitTransactions);
          // Show first 10 items
          setDisplayedTransactions(debitTransactions.slice(0, ITEMS_PER_PAGE));
        } else {
  ...
  const renderContent = () => {
    ...
    if (allTransactions.length === 0) {
      return <Text className="text-gray-500 text-center py-5">No withdrawals found.</Text>;
    }
        ...
        {hasMoreItems && (
          <View className="items-center py-4">
            ...
            <Text className="text-xs text-gray-500 mt-2">
              Showing {displayedTransactions.length} of {allTransactions.length} withdrawals
            </Text>
          </View>
        )}
      </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      {/* Header View */}
      <View className="flex-row justify-between items-center px-4 mt-10 py-3 bg-white border-b border-gray-200">
        <TouchableOpacity onPress={() => router.back()} className="w-8 h-8 justify-center items-center">
          <Ionicons name="arrow-back" size={24} color="black" />
        </TouchableOpacity>
        <Text className="text-lg font-bold text-gray-800">Withdrawal History</Text>
      ...
      <View className="flex-1 px-4 pt-2">
        <Text className="text-base font-bold text-gray-700 mb-2">All Withdrawals</Text>
